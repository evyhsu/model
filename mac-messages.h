...
//yen
    ...
    MESSAGE_TYPE_MOB_MSHO_REQ = 57,
    MESSAGE_TYPE_MOB_BSHO_RSP = 56,
    MESSAGE_TYPE_MOB_HO_IND = 59
  };
...
class MshoReq : public Header
{

public:
  MshoReq (void);
  virtual ~MshoReq (void);

  void SetReportMetric (uint8_t reportMetric);  
  void SetNNewBsIndex (uint8_t nNewBsIndex);
  uint8_t GetReportMetric (void) const;
  uint8_t GetNNewBsIndex (void) const;
...
private:
/** MSHO request frame */
  uint8_t m_reportMetric;
  uint8_t m_nNewBsIndex;
...
class MshoReqBsIndex : public Header
{
public:
  MshoReqBsIndex (void);
  virtual ~MshoReqBsIndex (void);

  void SetNeighborBsIndex (uint8_t neighborBsIndex);  
  void SetPreambleIndex (uint8_t preambleIndex);
  void SetBsCinrMean (uint8_t bsCinrMean);  
  void SetBsRssiMean (uint8_t bsRssiMean);
  void SetRelativeDelay (uint8_t relativeDelay);
...
private:
  ...
  uint8_t m_relativeDelay; //if report_metric[2]==1
  
  uint8_t m_serviceLevelPrediction: 3;
