                              ...
                              uint16_t protocolNumber)
{
  ...
  NS_LOG_INFO ("\t\tPaket Size:  " << packet->GetSize ());//getsize() Definition at packet.h. return m_buffer.GetSize ();
  NS_LOG_INFO ("\t\tProtocol:    " << protocolNumber);
//yen
  NS_LOG_INFO ("yen_SFID: " << serviceFlow << " CID : " << cid);
...
//yen
  NS_LOG_INFO (" yen_CID :Enqueue " << connection->GetCid ());
            ...
            case ManagementMessageType::MESSAGE_TYPE_MOB_MSHO_REQ:
              {
                ...
                //GetServiceFlowManager ()->AllocateServiceFlows (mshoreq, cid);
                break;
              }                             
...
//yen
  NS_LOG_INFO ("CID GetConnection :" << connection);
    }
