...
uint8_t
YenSSHandover::getMOB_MSHO_REQ_size (void)
{	

    std::cout << "yensshandover" << std::endl;
	
	if (m_n_new_bs_index !=0) 
    	m_size++;

	if (m_report_metric & 0x1) m_tmp++;
  	if (m_report_metric & 0x2) m_tmp++;
  	if (m_report_metric & 0x4) m_tmp++;
